...
initiative_col = "Existence of initiatives and projects"
metric_col = "Tourism Index"
hotels_col = "Total number of hotels"

# Allow filtering initiatives
initiatives = st.sidebar.multiselect(
    "Select initiative status",
    options=df[initiative_col].dropna().unique(),
    default=df[initiative_col].dropna().unique()
)

df_filtered = df[df[initiative_col].isin(initiatives)]

# --- Visualization 1: Average Tourism Index ---
st.subheader("Average Tourism Index by Initiative Status")

avg_tourism = (
    df_filtered.groupby(initiative_col)[metric_col]
    .mean()
    .reset_index()
...
fig1 = px.bar(
    ...
    x=initiative_col,
    y=metric_col,
    title="Average Tourism Index by Initiative Status",
    text_auto=".2f"
)
st.plotly_chart(fig1, use_container_width=True)

# --- Visualization 2: Distribution of Hotels ---
st.subheader("Distribution of Hotels by Initiative Status")

fig2 = px.box(
    df_filtered,
    x=initiative_col,
    y=hotels_col,
    color=initiative_col,
    title="Distribution of Total Number of Hotels"
)
st.plotly_chart(fig2, use_container_width=True)
